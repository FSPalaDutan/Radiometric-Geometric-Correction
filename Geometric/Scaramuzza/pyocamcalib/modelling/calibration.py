...
class CalibrationEngine:
    ...
    def detect_corners(self, check: bool = False, max_height: int = 520):
        ...
        for img_f in tqdm(sorted(images_path)):
            ...
            for block, bias in list(product(range(20, 40, 5), range(-10, 31, 5))):
                ...
                # Mensaje de advertencia si no se detectan esquinas
                logger.warning(f"No se pudieron detectar esquinas en la imagen: {img_f}")
    ...
    def save_detection(self):
        ...
        save_dir = os.path.join(os.path.dirname(__file__), '..', 'checkpoints', 'corners_detection')
        os.makedirs(save_dir, exist_ok=True)  # Crea el directorio si no existe
        save_path = os.path.join(save_dir, f'detections_{self.cam_name}_{dt_string}.pickle')
        with open(save_path, 'wb') as f:
            pickle.dump(self.detections, f)
            logger.info(f"Detection file saved with success to {save_path}.")
    ...
    def estimate_fisheye_parameters(self, grid_size: int = 5):
        ...
        # Identificar e imprimir imágenes no válidas
        invalid_images = [img_path for i, img_path in enumerate(sorted(self.detections.keys())) if not valid_pattern[i]]
        if invalid_images:
            logger.warning(f"Las siguientes imágenes no son válidas para la calibración: {invalid_images}")
        ...
        # Get all images in the working directory
        all_images = sorted([str(e) for e in get_files(self.working_dir)])

        # Create the 'errores' folder if it doesn't exist
        errors_dir = r"C:\Users\Franklin Leon\Documents\TESIS\Geometric Calibration\py-OCamCalib-main\errores"
        os.makedirs(errors_dir, exist_ok=True)

        # Save errors to a .txt file in the 'errores' folder
        error_file_path = os.path.join(errors_dir, f"errors_{self.cam_name}.txt")
        with open(error_file_path, "w") as f:
            valid_counter = 0  # Counter for valid images
            for i, img_path in enumerate(all_images, start=1):
                if img_path in self.detections:
                    # Find the index in sorted detections keys
                    detection_index = list(sorted(self.detections.keys())).index(img_path)
                    if self.valid_pattern[detection_index]:
                        # Image is valid; use valid_counter to index rms lists
                        mean = self.rms_mean_list[valid_counter]
                        std = self.rms_std_list[valid_counter]
                        f.write(f"Image {i}: Mean RMS = {mean:.2f}, Std RMS = {std:.2f}\n")
                        valid_counter += 1
                    else:
                        # Image has detections but is not valid
                        f.write(f"Image {i}: Mean RMS = 0.00, Std RMS = 0.00\n")
                else:
                    # Image has no detected corners
                    f.write(f"Image {i}: Mean RMS = 0.00, Std RMS = 0.00 (No se detectaron esquinas)\n")
        logger.info(f"Error file saved successfully to {error_file_path}")
    ...
    def save_calibration(self):
        ...
        save_dir = os.path.join(os.path.dirname(__file__), '..', 'checkpoints', 'calibration')
        os.makedirs(save_dir, exist_ok=True)  # Crea el directorio si no existe
        save_path = os.path.join(save_dir, f'calibration_{self.cam_name}.json')
        with open(save_path, 'w') as f:
            json.dump(outputs, f, indent=4)
            logger.info(f"Calibration file saved with success to {save_path}.")
