...
void GenericCameraCalibration::writeImageResidual(const ImageData & data, const string & fileName) const
{
    ...
    for (int transfIdx = 0; transfIdx < transfVec.size(); transfIdx++)
    {
        ...
        for (int i = 0; i < projectedVec.size(); i++)
        {
            cout << projectedVec[i].transpose() << "   err : " << errVec[i] << endl;
        }

        // Solo mostrar imagen si el parámetro está activado
        if (data.showOutliers)
        {
            Mat img;
            if (data.useImages)
                img = imread(data.imageNameVec[transfIdx], cv::IMREAD_COLOR);
            else
                img = Mat(data.imageHeight, data.imageWidth, CV_8UC3, Scalar(0, 0, 0));

            // Dibujar puntos detectados
            drawPoints(img, data.detectedCornersVec[transfIdx]);

            // Dibujar proyecciones en verde
            for (int i = 0; i < projectedVec.size(); i++)
            {
                Scalar color = Scalar(0, 255, 0); // Verde
                if (errVec[i] > 3.6 * sigma || errVec[i] > 1.0)
                    color = Scalar(0, 127, 255); // Naranja (outlier)
                circle(img, Point(projectedVec[i][0], projectedVec[i][1]), 9, color, 5);
            }
