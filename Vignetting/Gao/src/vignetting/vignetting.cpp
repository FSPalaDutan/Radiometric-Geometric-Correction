...
void
camera_model::vignetting::showResualt( const std::string& save_path )
{
    cv::Mat image;
    if ( getIs_color( ) )
    {
        ...
        for ( int index = 0; index < 3; ++index )
        {
            ...
            for ( int row_index = 0; row_index < getImageSize( ).height; ++row_index )
                for ( int col_index = 0; col_index < getImageSize( ).width; ++col_index )
                {
                    ...
                    if ( value > 255 )
                        value = 255;
                    img_tmp.at< uchar >( row_index, col_index ) = static_cast<uchar>(value);
                }
            images.push_back( img_tmp );
        }
        image = cv::Mat( getImageSize( ), CV_8UC3, cv::Scalar( 0, 0, 0 ) );
        cv::merge( images, image );
    }
    else
    {
        image = cv::Mat( getImageSize( ), CV_8UC1, cv::Scalar( 0 ) );
        for ( int row_index = 0; row_index < getImageSize( ).height; ++row_index )
            for ( int col_index = 0; col_index < getImageSize( ).width; ++col_index )
            {
                ...
                if ( value > 255 )
                    value = 255;
                image.at< uchar >( row_index, col_index ) = static_cast<uchar>(value);
            }
    }

    if ( !save_path.empty() )
    {
        std::string filename = save_path + "/resualt.png";
        cv::imwrite( filename, image );
        std::cout << "Imagen resualt guardada en: " << filename << std::endl;
    }
}
